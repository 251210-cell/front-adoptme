...
async function subirImagenABackend(file, callback) {
    ...
    try {
        // Enviar la imagen al backend real (reemplazar la URL cuando esté definido el backend)
        ...
        if (response.ok) {
            ...
            // Se asume que el backend responde con { url: "https://tudominio.com/imagen.jpg" }
            callback(data.url);
        } else {
            console.error('Error al subir la imagen:', response.statusText);
            Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo subir la imagen al servidor.' });
            callback(''); // En caso de error, no devuelve URL
        }
    } catch (error) {
        console.error('Error de red al subir la imagen:', error);
        Swal.fire({ icon: 'error', title: 'Sin conexión', text: 'Asegúrate de que el backend esté ejecutándose.' });
        callback('');
    }
}
